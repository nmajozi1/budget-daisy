    ...
    <dialog id="add_item_modal" className="modal">
      <div className="modal-box">
        ...
        <form action={action} className="flex-col w-full mt-4">
          <input type="text" name="item" placeholder="Enter budget item" className="input input-bordered w-full max-w-lg" />
          {state?.error && <ul className='list-disc text-red-600 pl-4'>{state.error.item?.map((error, index) => (<li key={index}>{error}</li>))}</ul>}
          <input type="text" name="description" placeholder="Enter budget item description" className="input input-bordered w-full max-w-lg mt-2" />
          {state?.error && <ul className='list-disc text-red-600 pl-4'>{state.error.description?.map((error, index) => (<li key={index}>{error}</li>))}</ul>}
          <input type="text" name="amount" placeholder="Enter item amount" className="input input-bordered w-full max-w-lg mt-2" />
