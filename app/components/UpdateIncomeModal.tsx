...
import { findIncomeItemByID, updateIncomeItem } from "../actions/income.action";

interface IID {
  id: string | undefined;
}

const UpdateIncomeModal = (props: IID) => {
  const {id} = props;
  ...
  const onOpenForm = async () => {
    (document.getElementById(`update_income_item_modal-${id}`) as HTMLDialogElement)?.showModal();
    const item = (await findIncomeItemByID(id)).message as string;
  ...
  const onUpdate = async () => {
    ...
    await updateIncomeItem(id, updateData);
  }
  ...
  return (
    <>
      ...
      <dialog id={`update_income_item_modal-${id}`} className="modal">
